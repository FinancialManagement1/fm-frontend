...
export default function ScannerScreen() {
  ...
  const handleImageSelect = async (source) => {
    ...
    try {
      ...
      console.log("🔴 SCAN ID FROM API:", apiResult?.scanId);

      console.log("SCAN RESULT:", apiResult);
  ...
  //////////////// Logic to determine final category to show (suggested vs manual vs default)
  ...
  const handleConfirm = async () => {
    ...
    const confirmData = {
      ...
      // ✅ force lowercase (backend expects exact)
      type: (scanData.suggestedType || "").toLowerCase(),

      // ✅ force number
      amount: Number(selectedAmount || scanData.amount),

      // ✅ trim + normalize category
      category: finalCategory,

      // ✅ fix date format (remove time if exists)
  ...
  const getConfidenceColor = (score) => {
    if (score >= 0.9) return theme.income; // Green
    if (score >= 0.7) return theme.accent; // Yellow
        ...
        <View style={styles.fieldContainer}>
          ...
          >
            <TextInput
              ...
              onChangeText={(text) =>
                setScanData({ ...scanData, merchant: text })
              }
        ...
        <View style={styles.fieldContainer}>
          ...
          <View style={styles.fieldInputContainer}>
            <TextInput
              ...
              onChangeText={(text) =>
                setScanData({ ...scanData, description: text })
              }
            ...
            >
              <Text
                style={[
                  ...
                  scanData.suggestedType === "expense" &&
